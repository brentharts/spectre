...
forked from: https://github.com/programjames/einstein_tiling
...
if '--blender' in sys.argv:
    cmd = ['blender', '--python-exit-code', '1', '--python-use-system-env', '--python', __file__]
    print(cmd)
...
try:
    ...
    print('note: to install pysat run: pip install python-sat --break-system-packages')
...
if __name__ == '__main__':
    ...
    for i, tile in enumerate(tiles):
        ...
        if bpy:
            path = poly.get_path()
            verts = [[v[0], v[1], 0] for v in path.vertices]
            faces = [list(range(len(verts)))]
            mesh = bpy.data.meshes.new('tile.%s' % i)
            mesh.from_pydata(verts, [], faces)
            mesh.update()
            obj = bpy.data.objects.new(mesh.name, mesh)
            obj.show_wire = True
            obj.color = list(colors[c[i]]) + [1]
    ...
    print(bez_points)
    if bpy:
        curve_data = bpy.data.curves.new(name='celtic-knot', type='CURVE')
        curve_data.dimensions = '3D'
        curve_data.bevel_depth = 0.03
        curve_data.bevel_resolution = 4
        curve_data.extrude = 0.25
        spline = curve_data.splines.new('BEZIER')
        spline.bezier_points.add(len(bez_points)-1)
        for i,p in enumerate(spline.bezier_points):
            x,y = bez_points[i]
            p.co.x = x
            p.co.y = y
            p.co.z = i * 0.01
            p.handle_right_type, p.handle_left_type = 'AUTO', 'AUTO'
